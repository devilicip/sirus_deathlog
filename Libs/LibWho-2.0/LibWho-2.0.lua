...
function lib:CancelPendingWhoNext()
    lib['frame']:Hide()
    lib.Timeout_time = nil -- Сбрасываем время таймаута
end
...
-- queue scheduler
local queue_weights = { [1] = 0.3, [2] = 0.2, [3] = 0.2 }
local queue_bounds = { [1] = 0.3, [2] = 0.2, [3] = 0.2 }
...
function lib:AskWhoNext()
    ...
    if self.WhoInProgress then
        ...
        self.Args = nil
        self.WhoInProgress = false -- Сбрасываем флаг выполнения запроса

        -- Очистка данных текущего запроса
        self.Result = nil
        self.Total = nil

        -- Повторное добавление в очередь (если требуется)
        if args and args.query then
        ...
        -- Сброс интервала
        queryInterval = math.min(queryInterval + 0.5, lib.MaxInterval)
...
function lib:ReturnWho()
	...
	if queryInterval > self.MinInterval then
		queryInterval = queryInterval - 0.2
...
function lib:WHO_LIST_UPDATE()
    if not self.WhoInProgress then
        dbg("Ignoring WHO_LIST_UPDATE during timeout")
        return
    end
...
function lib:ProcessWhoResults()
    if not self.WhoInProgress then
        dbg("Ignoring outdated WHO_LIST_UPDATE")
        return -- Прекращаем обработку, если запрос уже отменен
    end
